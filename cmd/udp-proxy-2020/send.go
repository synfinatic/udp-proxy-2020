...
type Send struct {
	...
	srcIP    net.IP          // source IP of the packet
...
func (s *SendPktFeed) Send(p gopacket.Packet, srcif string, srcIP net.IP, linkType layers.LinkType) {
	s.lock.Lock()
	for thisif, send := range s.senders {
		...
		send <- Send{packet: p, srcif: srcif, srcIP: srcIP, linkType: linkType}
	}
	s.lock.Unlock()
}
